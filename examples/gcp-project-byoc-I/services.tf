locals {
  required_project_services = toset(concat([
    ...
    ], var.enable_pd_kms || var.enable_gcs_kms || var.enable_gke_secrets_encryption || var.enable_cmek ? ["cloudkms.googleapis.com"] : [],
    var.enable_cmek ? ["iamcredentials.googleapis.com"] : [],
